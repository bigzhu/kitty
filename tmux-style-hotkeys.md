...
1. **已禁用自动启动 tmux**：修改了 `macos.conf` 和 `linux.conf`，不再自动启动 tmux
2. 重启 kitty 或使用 `Ctrl+a r` 重载配置
3. 所有热键与 tmux 保持一致，但使用 kitty 原生功能
4. 现在启动 kitty 时会直接进入 zsh，不会自动进入 tmux
5. 支持多种布局，比 tmux 更灵活

## 迁移说明

### 从 tmux 完全迁移到 kitty 原生功能
- ✅ 已禁用自动启动 tmux
- ✅ 所有常用 tmux 热键已映射
- ✅ 保持相同的工作流程
- ✅ 获得更好的性能和功能